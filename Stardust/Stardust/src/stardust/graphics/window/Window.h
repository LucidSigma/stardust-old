...
namespace stardust
{
	...
	public:
		...
		struct CreateInfo
		{
			...
			std::variant<int, Position> x = Position::Centred;
			std::variant<int, Position> y = Position::Centred;
			glm::uvec2 size;

			std::vector<CreateFlag> flags;

			std::optional<glm::uvec2> minimumSize = std::nullopt;
			std::optional<glm::uvec2> maximumSize = std::nullopt;
		};
	...
	private:
		...
		std::unique_ptr<SDL_Window, WindowDestroyer> m_handle = nullptr;
	...
	public:
		...
		[[nodiscard]] inline std::string GetTitle() const noexcept { return SDL_GetWindowTitle(GetRawHandle()); }
		inline void SetTitle(const std::string_view& title) const noexcept { SDL_SetWindowTitle(GetRawHandle(), title.data()); }
		...
		[[nodiscard]] inline std::uint32_t GetPixelFormat() const noexcept { return SDL_GetWindowPixelFormat(GetRawHandle()); }
		inline int GetDisplayIndex() const noexcept { return SDL_GetWindowDisplayIndex(GetRawHandle()); }
	...
	private:
		int GetWindowCoordinate(const std::variant<int, Position>& windowCoordinate) const;
	};
}
