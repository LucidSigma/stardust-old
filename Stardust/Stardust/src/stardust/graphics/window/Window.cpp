...
namespace stardust
{
	...
	Window::Window(Window&& other) noexcept
		: m_handle(nullptr), m_size(glm::uvec2{ 0u, 0u }), m_sizeBeforeFullscreen(std::nullopt), m_fullscreenType(FullscreenType::Hard), m_isFullscreen(false), m_hasUpdatedFullscreen(false)
	{
	...
	Window& Window::operator =(Window&& other) noexcept
	{
		...
		m_sizeBeforeFullscreen = std::exchange(other.m_sizeBeforeFullscreen, std::nullopt);
	...
	void Window::Initialise(const CreateInfo& createInfo)
	{
		...
		m_handle = std::unique_ptr<SDL_Window, WindowDestroyer>(SDL_CreateWindow(createInfo.title.data(), x, y, createInfo.size.x, createInfo.size.y, windowFlags));
	...
	int Window::GetWindowCoordinate(const std::variant<int, Position>& windowCoordinate) const
	{
		...
		{
			return std::get<int>(windowCoordinate);
		}
	}
}
